...
struct MainTabView: View {
    ...
    @AppStorage("hasSeedData") private var hasSeedData = false
    ...
    var body: some View {
        ...
        .onChange(of: authenticationViewModel.user) { _, user in
            guard let uid = user?.uid, !hasSeedData else { return }
            Task {
                try? await Vehicle.writeSeedData(userUID: uid)
                hasSeedData = true
            }
        }
    }
